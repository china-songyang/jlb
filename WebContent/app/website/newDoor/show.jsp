	...
	<script type="text/javascript">
		$(function() {
			...
			$('#queryTable').datagrid({
				...
				url : '<%=request.getContextPath()%>/app/website/newDoor/listNewDoor.action',
				idField : 'id',//唯一标识列
				sortName : 'id',
				sortOrder : 'desc',
				remoteSort : true,
				...
				toolbar : [ {
					...
					iconCls : 'icon-add',
					handler : function() {
						window.location.href='<%=request.getContextPath()%>/app/website/newDoor/add.jsp';
					...
					handler : function() {
						...
						$.messager.confirm('确认删除项', '确认删除该选项', function(result){
							if (result){
								window.location.href='<%=request.getContextPath()%>/app/website/newDoor/deleteNewDoor.action?ids='+ids.join('__');
							}
						});
						return false;
					}
		...
		function deleteVO(id){
			$.messager.confirm('确认删除项', '确认删除该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/website/newDoor/deleteNewDoor.action?ids=' + id;
				}
			});
			return false;
		}
		function editVO(id){
			window.location.href='<%=request.getContextPath()%>/app/website/newDoor/query.action?id='+ id;
			return false;
		}
	</script>
</head>
